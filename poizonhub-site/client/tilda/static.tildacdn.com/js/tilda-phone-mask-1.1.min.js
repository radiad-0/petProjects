...
function t_form_phonemask_load_one(recId) {
  var phoneMaskList = document.querySelectorAll(
    "#rec" + recId + " .js-phonemask-input"
  );
  Array.prototype.forEach.call(phoneMaskList, t_form_phonemask_load);
}
function t_form_phonemask_load(input) {
  if (
    (input instanceof Element || (input = input[0]),
    input && void 0 !== input.getAttribute)
  ) {
    var recId = input.getAttribute("data-phonemask-id"),
      lid = input.getAttribute("data-phonemask-lid"),
      iso = input.getAttribute("data-phonemask-maskcountry") || "";
    input.setAttribute("data-phonemask-init", "yes"),
      t_form_phonemask_init(recId, lid, iso);
  }
}
function t_form_phonemask_init(recId, lid, iso) {
  if (iso && "undefined" !== iso)
    ...
    t_form_phonemask_initPhoneMaskForm(recId, lid, window.geoCountry);
    ...
    xhr.open("GET", "https://geo.tildacdn.com/geo/country/", !0),
      (xhr.onreadystatechange = function () {
        if (4 === xhr.readyState)
          if (xhr.status >= 200 && xhr.status < 400) {
            (iso = xhr.responseText), (window.geoCountry = iso);
            var geoCountryLoaded = document.createEvent("Event");
            geoCountryLoaded.initEvent("geoCountryLoaded", !0, !1),
              document.dispatchEvent(geoCountryLoaded),
              t_form_phonemask_initPhoneMaskForm(recId, lid, iso);
          } else t_form_phonemask_initPhoneMaskForm(recId, lid, "");
      }),
      xhr.send(),
    ...
    if (
      (inputGroup ||
        (window.tildamode && "zero" === window.tildamode
          ? (inputGroup = rec.querySelector(
              '.tn-artboard [data-input-lid="' + lid + '"]'
            )) || (inputGroup = rec.querySelector(".tn-artboard"))
          : (inputGroup = document.querySelector(
              '[data-input-lid="' + lid + '"]'
            )) || (inputGroup = document.body)),
      !inputGroup.getAttribute("data-init-mask"))
    ) {
      ...
      if (input) {
        ("string" == typeof iso && iso) || (iso = ""),
          iso.length > 0 && (iso = iso.toLowerCase()),
          iso.length > 2 && (iso = "");
        var countries = t_form_phonemask__getCountriesList();
        if (countries[iso]) var country = countries[iso];
        else
          var country = {
        ...
        input.setAttribute("data-phonemask-iso", iso),
          input.setAttribute(
            "data-phonemask-code",
            iso.length > 0 ? country.c : ""
          ),
          input.setAttribute(
            "data-phonemask-mask",
            iso.length > 0 ? country.m : ""
          ),
          t_form_phonemask__calcMaxlength(input);
        var selectFlag = inputGroup.querySelector(
            ".t-input-phonemask__select-flag"
          ),
          selectCode = inputGroup.querySelector(
            ".t-input-phonemask__select-code"
          ),
          currentMask = input.getAttribute("data-phonemask-mask"),
          currentCode = input.getAttribute("data-phonemask-code"),
          optionsWrap;
        if (iso.length > 0) {
          ...
          selectFlag.setAttribute("data-phonemask-flag", iso),
            ...
            currentMask &&
              input.setAttribute(
                "placeholder",
                currentMask.substr(currentCode.length).replace(/^-+/, "")
              ),
            t_form_phonemask__changeMinLength(
        ...
        t_form_phonemask__addCurrentStyle(inputGroup),
          ...
          ).innerHTML = t_form_phonemask__getDrawSelector(countries)),
...
function t_form_phonemask__replaceInput(inputGroup) {
  ...
  if (input) {
    var name = input.getAttribute("name"),
      dataReq = input.getAttribute("data-tilda-req")
        ? 'data-tilda-req="1"'
        : "",
      borderBottom = !!input.classList.contains("t-input_bbonly"),
      style = input.getAttribute("style"),
      oldValue = input.value;
    input.outerHTML =
      '<div class="t-input t-input-phonemask__wrap" style="' +
      style +
      '"><div class="t-input-phonemask__select"><span class="t-input-phonemask__select-flag" data-phonemask-flag=""></span><span class="t-input-phonemask__select-triangle"></span><span class="t-input-phonemask__select-code"></span></div><div class="t-input-phonemask__options-wrap" data-nosnippet></div><input type="tel" class="t-input t-input-phonemask" name="tildaspec-phone-part[]" value="' +
      oldValue +
    ...
    borderBottom && wrap.classList.add("t-input_bbonly");
...
function t_form_phonemask__calcMaxlength(input) {
  var maskWithoutCode = input
    .getAttribute("data-phonemask-mask")
    .substr(input.getAttribute("data-phonemask-code").length);
  "-" === maskWithoutCode.charAt(0) &&
...
function t_form_phonemask__addCurrentStyle(inputGroup) {
  var inputWrap = inputGroup.querySelector(".t-input-phonemask__wrap"),
    input = inputGroup.querySelector(".t-input-phonemask"),
    inputBlock = inputGroup.querySelector(".t-input-block"),
    selectCode = inputGroup.querySelector(".t-input-phonemask__select-code"),
    color = inputWrap.style.color,
    fontSize = inputWrap.style.fontSize,
    fontWeight = inputWrap.style.fontWeight,
    fontFamily = inputWrap.style.fontFamily;
  (inputBlock.style.overflow = "visible"),
    "rgb(0, 0, 0)" !== color && (input.style.color = color),
    (input.style.fontSize = fontSize),
    (input.style.fontWeight = fontWeight),
    (input.style.fontFamily = fontFamily),
    (selectCode.style.fontSize = fontSize),
    (selectCode.style.fontWeight = fontWeight);
}
function t_form_phonemask__getDrawSelector(countries) {
  var str = "";
  for (var country in ((str += "<noindex>"), countries))
    (str +=
      ...
      '">'),
      (str +=
        '<div class="t-input-phonemask__options-name">' +
        countries[country].n +
        "</div>"),
      (str += '<div class="t-input-phonemask__options-right">'),
      (str +=
        '<span class="t-input-phonemask__options-code">' +
        countries[country].c +
        "</span>"),
      (str +=
...
function t_form_phonemask__initSelectEvents(rec, inputGroup, countries) {
  var cover = rec.querySelectorAll(".t-cover, .t396__artboard"),
    input = inputGroup.querySelector(".t-input-phonemask"),
    select = inputGroup.querySelector(".t-input-phonemask__select"),
    selectWrap = inputGroup.querySelector(".t-input-phonemask__options-wrap"),
    optionItems = inputGroup.querySelectorAll(
  ...
  function t_form_phonemask__toggleSelect() {
    selectWrap.classList.contains("t-input-phonemask__options-wrap_open")
      ? t_form_phonemask__activeSelectClose()
      : (t_form_phonemask__activeSelectClose(),
        selectWrap.classList.add("t-input-phonemask__options-wrap_open")),
      1 === cover.length && (cover[0].style.overflow = "unset");
  }
  function t_form_phonemask__activeSelectClose() {
    var activeSelect = document.querySelector(
      ".t-input-phonemask__options-wrap_open"
    );
    activeSelect &&
      activeSelect.classList.remove("t-input-phonemask__options-wrap_open");
  }
  function t_form_phonemask__changeInputVal() {
    ...
    ("(" !== value && value) ||
      ...
      this.setAttribute("data-phonemask-current", value),
      t_form_phonemask__addNumberMask(
        t_form_phonemask__copypasteHandling(
          this,
          this.getAttribute("data-phonemask-without-code")
        ),
        this
      ),
      t_form_phonemask__changeVal(inputGroup, input),
      "+7" === this.getAttribute("data-phonemask-code") &&
        1 === this.value.indexOf("89") &&
        (this.value = this.value.replace("89", "9"));
  }
  function t_form_phonemask__changeOption() {
    t_form_phonemask__chooseCountry(inputGroup, this),
      t_form_phonemask__calcMaxlength(input),
      t_form_phonemask__activeSelectClose(),
      1 === cover.length && (cover[0].style.overflow = "");
  }
  function t_form_phonemask__getOutsideClick(e) {
    var elem = e.target.closest(".t-input-phonemask__options-wrap"),
      select = e.target.closest(".t-input-phonemask__select");
    elem ||
      select ||
      (t_form_phonemask__activeSelectClose(),
      1 === cover.length && (cover[0].style.overflow = ""));
  ...
  function t_form_phonemask__handlerKeyUp(e) {
    ...
    if (activeSelect) {
      var regExp = /[-[\]/{}()*+?.\\^$|]/g,
        itemChosen = activeSelect.querySelector(
          ".t-input-phonemask__options-item_chosen"
        ),
        searchCountry = [];
      if (
        (itemChosen &&
          itemChosen.classList.remove("t-input-phonemask__options-item_chosen"),
        (e.keyCode >= 48 && e.keyCode <= 57) ||
          (e.keyCode >= 96 && e.keyCode <= 105))
      ) {
        for (var country in ((word = (word += e.key).replace(regExp, "\\$&")),
        countries))
          t_form_phonemask__searchCountry(
            activeSelect,
            word,
            searchCountry,
            country,
        ...
        t_form_phonemask__scrollToCountry(activeSelect, searchCountry);
...
function t_form_phonemask__copypasteHandling(input, maskWithoutCode) {
  var currentValue = input.value,
    valueArr = currentValue.match(/[0-9]/g)
      ? currentValue.match(/[0-9]/g)
      : currentValue;
  return (
    "object" == typeof valueArr &&
      valueArr.length > maskWithoutCode.match(/[0-9]/g).join("").length &&
      (currentValue = valueArr
        .splice(
          input.getAttribute("data-phonemask-code").match(/[0-9]/g).length
        )
        .join("")),
    currentValue
  );
...
function t_form_phonemask__addNumberMask(val, input) {
  var inputCode = input.getAttribute("data-phonemask-code"),
    inputMask = input
      .getAttribute("data-phonemask-mask")
      .substr(inputCode.length),
    number = val.match(/\d+/g),
    numberArr = (number = number ? number.join("") : "").split(""),
    maskArr = inputMask.match(/(\+|\d+|[\s()-]|9+)/g),
    phoneString = "";
  if (maskArr) {
    ...
    (phoneString = (phoneString = phoneString.replace(/[\s()-]+$/, "")).replace(
      /^-+/,
      ""
    )),
...
function t_form_phonemask__changeVal(inputGroup, input) {
  input.value &&
    (inputGroup.querySelector(".js-phonemask-result").value =
      input.getAttribute("data-phonemask-code") + " " + input.value);
}
function t_form_phonemask__chooseCountry(inputGroup, option) {
  var input = inputGroup.querySelector(".t-input-phonemask"),
    selectCode = inputGroup.querySelector(".t-input-phonemask__select-code"),
    selectFlag = inputGroup.querySelector(".t-input-phonemask__select-flag"),
    currentCode = option.getAttribute("data-phonemask-code"),
    currentMask = option.getAttribute("data-phonemask-mask"),
    currentCountryCode = option.getAttribute("data-phonemask-country-code"),
    currentInput = input.getAttribute("data-phonemask-current");
  input.setAttribute("data-phonemask-iso", currentCountryCode),
    input.setAttribute("data-phonemask-code", currentCode),
    input.setAttribute("data-phonemask-mask", currentMask);
  var placeholder = currentMask.substr(currentCode.length).replace(/^-+/, "");
  ...
  input.focus(),
    currentInput && t_form_phonemask__addNumberMask(currentInput, input),
    t_form_phonemask__changeVal(inputGroup, input),
    t_form_phonemask__changeMinLength(placeholder, currentCode, inputGroup);
}
...
function t_form_phonemask__scrollToCountry(container, searchCountry) {
  if (0 !== searchCountry.length) {
    var searchCountrySorted,
      activeSelectTop = Array.prototype.sort
        .call(searchCountry, function (a, b) {
          return (
            a.dataset.phonemaskCode.length - b.dataset.phonemaskCode.length
          );
        })[0]
        .getBoundingClientRect().top,
      containerTop = container.getBoundingClientRect().top,
      calcScroll = document.body.scrollTop + container.scrollTop;
...
function t_form_phonemask__changeMinLength(mask, code, inputGroup) {
  ...
  if (mask && code) {
    var spaceLength = 1,
      input = inputGroup.querySelector(".t-input-phonemask"),
      inputResult = inputGroup.querySelector(".js-phonemask-result"),
      phonemaskCode = input.getAttribute("data-phonemask-code"),
      floatingLength = {
        "+49": 1,
        "+372": 1,
        "+355": 1,
    ...
    minlength =
      code.length + 1 + mask.length - (floatingLength[phonemaskCode] || 0);
  }
