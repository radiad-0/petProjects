...
(window.t_forms__lang = (
  ...
  t_forms__onReady(function () {
    ...
    !isST105 ||
      ((lang = isST105.getAttribute("data-tilda-project-lang")) &&
        (window.t_forms__lang = lang)),
      t_forms__initForms();
  ...
  (window.tildaForm.validate = function (form) {
    ...
    for (
      var isMinOrderSetted,
        isMinQuantitySetted,
        inputs = form.querySelectorAll(".js-tilda-rule"),
        arrError = [],
        isEmptyValue = !0,
        i = 0;
      i < inputs.length;
      ...
      var isEmpty = !value.length && !valueNoSpace.length,
        isCheckBoxOrRadio =
          "checkbox" === isCheckBoxOrRadio || "radio" === isCheckBoxOrRadio,
        isChecked = !form.querySelectorAll('[name="' + isChecked + '"]:checked')
          .length;
        ...
        switch (dataRule) {
          ...
          case "url":
            (regExp =
              ...
              value.length &&
                ((strValue = (strValue =
                  (strValue = value.split("//")) && 1 < strValue.length
                    ? strValue[1]
                    : strValue[0]).split("/")) &&
                strValue.length &&
                strValue[0]
          ...
          case "date":
            0 < valueNoSpace.length &&
              !valueNoSpace.match(
                ...
                }[input.getAttribute("data-tilda-dateformat")] ||
    ...
    return (
      "y" === dataFormCart &&
        ((isMinOrderSetted =
          void 0 !== window.tcart_minorder && 0 < window.tcart_minorder),
        (isMinQuantitySetted =
  ...
  (window.tildaForm.showErrorInPopup = function (form, arrErrors) {
    ...
    function t_forms__hidePopup(errorInputs) {
      "INPUT" ===
        (
          (errorInputs = errorInputs || window.event).target ||
          errorInputs.srcElement
        ).tagName &&
      ...
      if (arrErrors[i] && arrErrors[i].obj) {
        ...
        el instanceof Element || (el = el[0]),
          ...
          inputGroup &&
            ((errorInputs = inputGroup.querySelectorAll(".t-input-error")),
            t_addClass(inputGroup, "js-error-control-box"),
            0 < errorInputs.length && (isErrorBox = !0));
          ...
          if (
            ...
            for (var k = 0; k < errorItem.length; k++)
              errorItem[k].getAttribute("data-rule-filled") ||
                ((errorItem[k].textContent && errorItem[k].innerText) ||
              ...
              if ("{" == data.substring(0, 1)) {
                ...
                if ("object" == typeof objData)
                  if (objData && objData.message && "OK" !== objData.message)
                    title.innerHTML = objData.message;
                  else if (objData && objData.error) {
                    var errorBoxes = (msgContainers =
...
function t_forms__removePlaceholder(inputId) {
  ...
  "INPUT" === input.tagName &&
    ...
    (inputId = ""),
    form
      ? (inputId = form.getAttribute("data-input-lid"))
...
function t_forms__addPlaceholder(inputId) {
  ...
  inputGroup
    ...
    : (strPlace = input.closest("form")) &&
      (inputId = strPlace.getAttribute("data-input-lid"));
...
function t_forms__addInputItsGood(allForms) {
  allForms = allForms.querySelectorAll(
    ".js-form-proccess[data-formactiontype]"
  );
...
function t_forms__initBtnClick(formKey) {
  var formType =
      (formKey = formKey || window.event).target || formKey.srcElement,
    btnSubmit = !!formType.closest('[type="submit"]') && formType;
  if (btnSubmit) {
    var form = btnSubmit.closest(".js-form-proccess");
    if (form) {
      formKey.preventDefault
        ? formKey.preventDefault()
        : (formKey.returnValue = !1);
      var formId = form.getAttribute("id"),
        formType = [],
        formKey = "";
      if (
        (btnSubmit.tildaSendingStatus &&
          (formKey = btnSubmit.tildaSendingStatus),
        !(
          (formKey && 1 <= formKey) ||
        ...
        if (
          ...
          (formType = window.tildaForm.validate(form)),
          window.tildaForm.showErrors(form, formType))
        )
          return (
            t_removeClass(btnSubmit, "t-btn_sending"),
...
function t_forms__submitEvent(event) {
  ...
  event.target && (btnSubmit = event.target),
    btnSubmit &&
      ((btnStatus = ""),
      (btnSubmit = btnSubmit.querySelector('[type="submit"]')) &&
        btnSubmit.tildaSendingStatus &&
...
function checkVerifyTildaCaptcha(capthaKey) {
  if (
    -1 !==
    (capthaKey = capthaKey || window.event).origin.indexOf(
      window.tildaForm.endpoint
    )
  ) {
    var form = document.getElementById("js-tildaspec-captcha"),
      formCaptchaBox = document.getElementById("tildaformcaptchabox");
    if ("closeiframe" == capthaKey.data)
      return (
        formCaptchaBox && t_removeEl(formCaptchaBox),
        void (form && t_removeEl(form))
      );
    capthaKey = capthaKey.data;
    form &&
      ((form.value = capthaKey),
      formCaptchaBox && t_removeEl(formCaptchaBox),
      (form = form.closest("form")) && t_forms__submitEvent(form));
  }
}
function t_parseScripts(main, scripts) {
  ...
  Array.prototype.forEach.call(scripts, function (oldScript) {
    for (
      var script, newScript = document.createElement("script"), j = 0;
    ...
    oldScript.innerHTML.length
      ...
      : (((script = document.createElement("script")).src =
          oldScript.attributes.src.value),
...
function t_forms__onSuccess(form) {
  ...
  var prevBtn = form.closest(".r"),
    ...
    isScroll = !0,
    target =
      960 < window.innerWidth ? successBoxOffset - 200 : successBoxOffset - 100,
  ...
  successUrl &&
    ...
    (835 != recType && 862 != recType) ||
      ((prevBtn = prevBtn.querySelector(".t835__btn_prev")) &&
  ...
  for (
    ...
    if (
      !(
        !elements[i].name ||
        elements[i].disabled ||
        -1 < ["file", "reset", "submit", "button"].indexOf(elements[i].type)
      )
    )
      if ("select-multiple" !== elements[i].type)
        (-1 < ["checkbox", "radio"].indexOf(elements[i].type) &&
          !elements[i].checked) ||
...
function t_fadeIn(el) {
  ...
  "block" !== el.style.display &&
    ...
    (timer = setInterval(function () {
      (el.style.opacity = opacity),
        1 <= (opacity += 0.1) && clearInterval(timer);
    }, 30)));
}
